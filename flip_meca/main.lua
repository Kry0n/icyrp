...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job.name == 'mecanico' and not registed then
			RegisterCommands()
			registed = true
		else
			registed = false
...
RegisterNetEvent('inrp_mecanicojob:attachProp')
AddEventHandler('inrp_mecanicojob:attachProp', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
...
Citizen.CreateThread( function()
    while true do 
		Citizen.Wait(10)
		if ((IsDisabledControlJustPressed(0, dropkey) or (GetHashKey("WEAPON_UNARMED") ~= GetSelectedPedWeapon(GetPlayerPed(-1)))) and (closestEntity ~= 0) and (PlayerData ~= nil) and (PlayerData.job.name == 'mecanico')) then
...
RegisterNetEvent('inrp_mecanicojob:attachItem')
AddEventHandler('inrp_mecanicojob:attachItem', function(item)
    TriggerEvent("inrp_mecanicojob:attachProp",attachPropList[item]["model"], attachPropList[item]["bone"], attachPropList[item]["x"], attachPropList[item]["y"], attachPropList[item]["z"], attachPropList[item]["xR"], attachPropList[item]["yR"], attachPropList[item]["zR"])
end)
...
AddEventHandler('attach:prop_cs_trolley_01', function()
	holdAnim()
    TriggerEvent("inrp_mecanicojob:attachItem","prop_cs_trolley_01")
end)
...
AddEventHandler('attach:prop_cs_cardbox_01', function()
	holdAnim()
    TriggerEvent("inrp_mecanicojob:attachItem","prop_cs_cardbox_01")
end)

RegisterNetEvent('inrp_mecanicojob:removeall')
AddEventHandler('inrp_mecanicojob:removeall', function()
...
function OpenmecanicoActionsMenu()
	local playerPed = PlayerPedId()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecanico_actions', {
		title    = _U('mecanico'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mecanico', vehicleProps)
				...
				if Config.EnablePlayerManagement and PlayerData.job and (grade == 'boss' or grade == 'chief' or grade == 'einrp_mecanicojoberimente') then
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'mecanico', function(data, menu)
				menu.close()
			end)
		end
	end, function(data, menu)
		...
		CurrentAction     = 'mecanico_actions_menu'
		CurrentActionData = {}
	end)
end

function OpenmecanicoHarvestMenu()
	local grade = PlayerData.job.grade_name
	if Config.EnablePlayerManagement and PlayerData.job.name == 'mecanico' then
		local elements = {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecanico_harvest', {
			title    = _U('harvest'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			menu.close()
			if not holdingPackage then
				TriggerServerEvent('esx_mecanicojob:startHarvest',data.current.value)
			else
				exports['mythic_notify']:'seed_weed'('error', "Já tens algo na mão") -- mythic_notify system
			end
		end, function(data, menu)
			menu.close()
			CurrentAction     = 'mecanico_harvest_menu'
			CurrentActionData = {}
		end)
	else
...
function OpenmecanicoCraftMenu()
	if Config.EnablePlayerManagement and PlayerData.job.name == 'mecanico' then
		local elements = {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mecanico_craft', {
			title    = _U('craft'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_mecanicojob:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_mecanicojob:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_mecanicojob:startCraft3')
			end
		end, function(data, menu)
			menu.close()

			CurrentAction     = 'mecanico_craft_menu'
			CurrentActionData = {}
		end)
	else
...
function RegisterCommands()
	PlayerData = ESX.GetPlayerData()
	if PlayerData.job.name == 'mecanico' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > Config.DrawDistance then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mecanico', _U('mecanico'), amount)
		...
		RegisterCommand("prop_cs_trolley_01", function(source, args, raw)
			...
			if arg ~= nil then
				TriggerEvent("inrp_mecanicojob:removeall")
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_mecanicojob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mecanicojob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_mecanicojob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mecanicojob:putStockItems', itemName, count)
...
RegisterNetEvent('esx_mecanicojob:onHijack')
AddEventHandler('esx_mecanicojob:onHijack', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mecanicojob:onCarokit')
AddEventHandler('esx_mecanicojob:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mecanicojob:onFixkit')
AddEventHandler('esx_mecanicojob:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('esx_mecanicojob:hasEnteredMarker', function(zone)
	

	if zone == 'mecanicoActions' then
		CurrentAction     = 'mecanico_actions_menu'
		CurrentActionData = {}
...
AddEventHandler('esx_mecanicojob:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('esx_mecanicojob:stopCraft')
		TriggerServerEvent('esx_mecanicojob:stopCraft2')
		TriggerServerEvent('esx_mecanicojob:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('esx_mecanicojob:stopHarvest')
	end
...
Citizen.CreateThread(function()
	...
	AddTextComponentString(_U('mecanico'))
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'mecanico' then
...
Citizen.CreateThread(function()
	while true do
		...
		if Config.EnableEmploy and PlayerData.job then
			...
			if (GetDistanceBetweenCoords(coords,Config.Employ.Coords.x,Config.Employ.Coords.y,Config.Employ.Coords.z) < 3.5) and (PlayerData.job.name ~= 'mecanico') then
				for i=1,#Config.Employ.Workers do 
					if Config.Employ.Workers[i].identifier == PlayerData.identifier then
						...
						if IsControlJustReleased(0, 38) then
							TriggerServerEvent('esx_mecanicojob:setJob',PlayerData.identifier,'mecanico',Config.Employ.Workers[i].grade)
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'mecanico' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_mecanicojob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_mecanicojob:hasExitedMarker', LastZone)
			end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_mecanicojob:removeSpecialContact', 'mecanico')

		if Config.MaxInService ~= -1 then
			TriggerServerEvent('esx_service:disableService', 'mecanico')
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'mecanico' then

				if CurrentAction == 'mecanico_actions_menu' then
					OpenmecanicoActionsMenu()
				elseif CurrentAction == 'mecanico_harvest_menu' then
					OpenmecanicoHarvestMenu()
				elseif CurrentAction == 'mecanico_craft_menu' then
					OpenmecanicoCraftMenu()
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'mecanico', vehicleProps)
...
function OpenLSAction()
	...
	if IsControlJustReleased(0, 38) and not lsMenuIsShowed then
		if ((PlayerData.job ~= nil and PlayerData.job.name == 'mecanico') or Config.IsmecanicoJobOnly == false) then
			lsMenuIsShowed = true
    ...
    while true do
        ...
        for _, menuConfig in pairs(menuConfigs) do
            ...
            if menuConfig:enableMenu() then
				...
				if ((IsControlJustReleased(0, keybindControl)) and (PlayerData.job.name == 'mecanico')) then
					-- Init UI
					showMenu = true
					SendNUIMessage({
						type = 'init',
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_mecanicojob:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)

RegisterNetEvent('esx_mecanicojob:installMod')
AddEventHandler('esx_mecanicojob:installMod', function()
	local coords 		= GetEntityCoords(GetPlayerPed(-1))
	local vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 3.0, false, 23)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_mecanicojob:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_mecanicojob:cancelInstallMod')
AddEventHandler('esx_mecanicojob:cancelInstallMod', function()
	local coords 		= GetEntityCoords(GetPlayerPed(-1))
	local vehicle  		= GetClosestVehicle(coords.x, coords.y, coords.z, 3.0, false, 23)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			if k == data.current.modType or isRimMod then
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_mecanicojob:installMod')
				else
					...
					if data.current.price ~= nil then
						if isRimMod then
							...
							TriggerServerEvent("esx_mecanicojob:buyMod", price)
		...
		menu.close()
		TriggerEvent('esx_mecanicojob:cancelInstallMod')
		local playerPed = PlayerPedId()
		local coords 		= GetEntityCoords(GetPlayerPed(-1))
		local vehicle  		= GetClosestVehicle(coords.x, coords.y, coords.z, 3.0, false, 23)
...
Citizen.CreateThread(function()
    while true do
		...
		if ((GetDistanceBetweenCoords(Config.Zones.CarLift.Pos.x, Config.Zones.CarLift.Pos.y, Config.Zones.CarLift.Pos.z, GetEntityCoords(GetPlayerPed(-1), false).x, GetEntityCoords(GetPlayerPed(-1), false).y, GetEntityCoords(GetPlayerPed(-1), false).z - 1) < Config.DrawDistance) and (PlayerData.job.name == 'mecanico')) then
