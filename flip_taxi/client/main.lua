...
AddEventHandler('esx_aiTaxi:callTaxi', function(coords)
	if customer then
		ESX.ShowHelpNotification('Já existe um táxi a caminho')
	else
		customer = coords
		-- get best spawnpoint
		...
		if customer ~= nil then
			...
			if vehicle == globalTaxi then
				...
				if not DoesBlipExist(waypoint) and not onTour then
					ESX.ShowHelpNotification('Para onde queres ir?')
					Citizen.Wait(2000)
				else
			...
			if customer ~= nil then
				...
				if distanceMeTaxi <= 40 then
					if not parkingDone then
						...
						TriggerEvent('esx:showNotification', 'Seu taxi esta aí.')
					end
	...
	while true do
		...
		if customer ~= nil then
			if taxiArrived and not inCar and not onWayBack then
				ESX.ShowHelpNotification('Pressiona ~INPUT_PICKUP~ para entrar')
...
function atTarget(cancel)
	...
	if cancel then
		...
		local vehicle = GetVehiclePedIsIn(playerPed, false)
		if vehicle ~= globalTaxi then
			TriggerEvent('esx:showNotification', 'O táxi foi cancelado')
			cancelTaxi = true
		else
			TriggerEvent('esx:showNotification', 'O táxi não pode mais ser cancelado')
			return
		end
	end
	if not cancelTaxi then
		ESX.ShowHelpNotification('Atingimos nosso objetivo')
...
function drive(x, y , z, delete, status)
	if status == 'start' then
		...
		ESX.ShowHelpNotification('Um motorista está a caminho de você.')
	elseif status == 'end' then
		ESX.ShowHelpNotification('Obrigado por sua confiança.')
	end
