...
AddEventHandler("defib:useDefib", function(ambulancesConnected)
  ...
  if ambulancesConnected > 0 then
    ESX.ShowNotification("Médico disponível, entre em contato com uma ambulância o mais rápido possível!")
    ...
    ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(qtty)
      if qtty > 0 then
        ...
        if health == 0 then
          ...
          ESX.ShowNotification("Reanimação em andamento")
          ...
          ESX.ShowNotification("Essa pessoa não precisa de um desfibrilador")
        end
      else
        ESX.ShowNotification("Você não tem um desfibrilador com você")
      end
    end, defibItemName)
  end
end)
