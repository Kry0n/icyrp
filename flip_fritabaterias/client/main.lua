...
RegisterNetEvent('esx_enginekiller:onUse')
AddEventHandler('esx_enginekiller:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if Config.IgnoreAbort then
				TriggerServerEvent('esx_enginekiller:removeKiller')
			end
			TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
            ESX.ShowNotification(_U('Prying_open_bonnet'))
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					ESX.ShowNotification(_U('finished_killing'))
		...
		Citizen.CreateThread(function()
			...
			if CurrentAction ~= nil then
				...
				if IsControlJustReleased(0, Keys["X"]) then
					...
					ESX.ShowNotification(_U('aborted_killer'))
					CurrentAction = nil
				end
			end
