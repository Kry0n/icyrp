...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Precisas de: pistola ou pistola de combate ou  de sns pistol!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("Ninguem perto para fazer de refem!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Pressiona [G] para solar, [H] para matar")
